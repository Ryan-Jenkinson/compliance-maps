...
async function initFavorites() {
  ...
  var [foldersRes, savedRes] = await Promise.all([
    sb.from('article_folders').select('id,name,parent_id').eq('user_id', profile.id).order('name').limit(200),
    sb.from('saved_articles').select('id,article_url,folder_id,notes').eq('user_id', profile.id).limit(500),
  ]);

  _folders = foldersRes.data || [];
  _savedMap = {};
  (savedRes.data || []).forEach(function(r) {
    _savedMap[r.article_url] = { id: r.id, folder_id: r.folder_id, notes: r.notes || null };
  });
  ...
  topFolders.forEach(function(folder) {
    ...
    if (subfolders.length) {
      subfolders.forEach(function(sf) {
        ...
        html += '<a class="rail-sub-link" href="saved.html?folder=' + escHtml(sf.id) + '" style="padding-left:52px;display:flex;justify-content:space-between;">'
...
function _ensureFolderPicker() {
  ...
  _folderPicker.style.cssText = 'display:none;position:absolute;z-index:9001;background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:8px 0;min-width:220px;box-shadow:0 4px 24px rgba(0,0,0,0.5);';
...
function _buildPickerContent(picker, url) {
  ...
  // Notes section
  var existingNote = '';
  if (_savedMap[url] && _savedMap[url].notes) existingNote = _savedMap[url].notes;
  html += '<div style="border-top:1px solid var(--border);margin:6px 0;"></div>';
  html += '<div style="padding:6px 12px 8px;">';
  html += '<div style="font-family:var(--mono);font-size:10px;color:var(--text-muted);letter-spacing:1px;margin-bottom:6px;">NOTES</div>';
  html += '<textarea id="picker-notes" placeholder="Add a note…" rows="3"'
       + ' style="width:100%;background:var(--bg);border:1px solid var(--border);border-radius:4px;'
       + 'color:var(--text-primary);padding:6px 8px;font-size:12px;font-family:var(--sans);resize:none;box-sizing:border-box;"'
       + ' onkeydown="event.stopPropagation()">'
       + escHtml(existingNote)
       + '</textarea>';
  html += '<button onclick="_savePickerNotes()" style="margin-top:6px;background:none;border:1px solid var(--border);border-radius:4px;padding:4px 12px;font-family:var(--mono);font-size:11px;color:var(--text-muted);cursor:pointer;letter-spacing:1px;">SAVE NOTE</button>';
  html += '</div>';

  picker.innerHTML = html;
}

async function _savePickerNotes() {
  var textarea = document.getElementById('picker-notes');
  if (!textarea) return;
  var notes = textarea.value.trim() || null;
  var url = _pickerUrl;
  if (!_savedMap[url]) {
    var btn = document.getElementById('picker-notes') && document.getElementById('picker-notes').nextElementSibling;
    if (btn) { btn.textContent = 'SAVE ARTICLE FIRST'; setTimeout(function() { if (btn) btn.textContent = 'SAVE NOTE'; }, 2000); }
    return;
  }
  var r = await getClient().from('saved_articles').update({ notes: notes }).eq('id', _savedMap[url].id);
  if (!r.error) {
    _savedMap[url].notes = notes;
    var btn = textarea.nextElementSibling;
    if (btn) { btn.textContent = 'SAVED'; setTimeout(function() { if (btn) btn.textContent = 'SAVE NOTE'; }, 1200); }
  }
}
...
async function _saveArticle(url, title, topic, folderId, btnEl) {
  ...
  if (!r.error && r.data) {
    _savedMap[url] = { id: r.data.id, folder_id: folderId || null, notes: null };
...
function _ensureSendPanel() {
  ...
  _sendPanel.style.cssText = 'display:none;position:absolute;z-index:9001;background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:14px 16px;min-width:280px;box-shadow:0 4px 24px rgba(0,0,0,0.4);';
...
async function _populateSendRecipients() {
  if (!_colleagues) {
    var r = await getClient().from('profiles').select('id,email,full_name').limit(50);
...
async function sendArticle() {
  ...
  if (r.error) { msg.textContent = 'Failed \u2014 try again.'; msg.style.color = 'var(--red)'; msg.style.display = 'block'; }
    ...
    var senderName = profile.full_name || '';
